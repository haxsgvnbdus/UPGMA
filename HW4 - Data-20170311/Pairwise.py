...
def main(argv):
    try:
        opts, args = getopt.getopt(argv, "hg:s:f:")
    ...
    for opt, arg in opts:
        ...
        if opt == '-s':
            if os.path.isfile(arg):
                ...
                print "Error ! input file must exist for -s. Correct Usage:"
                usage()
                sys.exit(2)
        if opt == '-f':
            if os.path.isfile(arg):
            ...
            # If the input file given does not exist, print an error message and exit the program
                ...
                print "ERROR! Input file must exist for -f. Correct usage:"
                usage()
                sys.exit(2)
...
def part1(sequences,gap):
    i = 0
    while i < len(sequences) - 1:
        DP_table = makeArray(sequences[i], sequences[i + 1],gap)
    ...
    for i in range(len(sequences)):
        for j in range(len(sequences)):
            if i == j:  # comparing itself, the distance scores 0
                ...
                DP_table = makeArray(sequences[i], sequences[j],gap)
